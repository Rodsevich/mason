      ...
      overridePrint(() async {
        const expectedPrintLogs = <String>[
          'Generate code using an existing brick template.\n'
              ...
              '\n'
              '    --no-ai                     Skip AI pre-resolution. `| ai` tags survive into the output.\n'
              '    --refresh-ai                Bypass AI cache reads (still writes new entries).\n'
              '    --no-cache-ai               Disable both reads and writes of the AI cache for this run.\n'
              '    --use-mock-ai               Use the mock AI provider (reads brick_test/ai_fixtures.yaml). For testing bricks without contacting a real provider.\n'
              '    --ai-provider               Override the active AI provider for this run.\n'
              '    --ai-concurrency            Maximum simultaneous AI invocations.\n'
              '                                (defaults to "4")\n'
              '    --dry-run-ai                Run AI pre-resolution but DO NOT write any files. Reports what would be generated. Implies --no-hooks.\n'
              '    --ai-token-budget           Per-tag input token budget (heuristic chars/4). When set, aborts the render with a clear error before invoking the provider if any tag exceeds the budget.\n'
              '\n'
