  ...
  group('masonex bundle', () {
    ...
    group('git', () {
      test('creates a new universal bundle from git', () async {
        final testDir = Directory(path.join(Directory.current.path, 'git'))
          ..createSync(recursive: true);
        Directory.current = testDir.path;
      ...
      test('exits with code 64 when no git url is provided', () async {
        ...
        expect(result, equals(ExitCode.usage.code));
        verify(() => logger.err('A repository url must be provided')).called(1);
        verifyNever(() => logger.progress(any()));
      });
    ...
    group('hosted', () {
      test('creates a new universal bundle from hosted', () async {
        final testDir = Directory(path.join(Directory.current.path, 'hosted'))
          ..createSync(recursive: true);
        Directory.current = testDir.path;
        final result = await commandRunner.run([
          'bundle',
      ...
      test('exits with code 64 when no brick name is provided', () async {
        ...
        expect(result, equals(ExitCode.usage.code));
        verify(() => logger.err('A brick name must be provided')).called(1);
        verifyNever(() => logger.progress(any()));
      });
