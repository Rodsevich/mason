...
''');
      ...
      // Merge YAML
      final yamlMergeFile = File(p.join(brickTemplateDir.path, '>>>config.yaml'));
...
''');
      ...
      // In-file generation
      final inFileSnippet = File(p.join(brickTemplateDir.path, '%imports%.dart'));
      inFileSnippet.writeAsStringSync("import 'package:{{name}}/{{name}}.dart';");

      final outputDir = Directory(p.join(tempDir.path, 'output'))..createSync();

      // Pre-existing files for merging
      File(p.join(outputDir.path, 'data.json'))
...
settings:
  feature_y: false
''');
      File(p.join(outputDir.path, 'log.txt'))
...
''');
      ...
      // Verify YAML Merge
      final configYaml = File(p.join(outputDir.path, 'config.yaml')).readAsStringSync();
      ...
      final logContent = File(p.join(outputDir.path, 'log.txt')).readAsStringSync();
      expect(
        logContent,
        equals(
      ...
      final mainContent = File(p.join(outputDir.path, 'main.dart')).readAsStringSync();
      expect(
        mainContent,
        contains("import 'package:masonex_test/masonex_test.dart';\n// @GenerateBefore('imports')"),
      );
    });

    test('verifies conditional skip', () async {
      final brickDir = Directory(p.join(tempDir.path, 'brick_skip'))..createSync();
...
''');
      final brickTemplateDir = Directory(p.join(brickDir.path, '__brick__'))
        ..createSync();
      File(p.join(brickTemplateDir.path, '?show?file.txt'))
          .writeAsStringSync('Content');

      final outputDir = Directory(p.join(tempDir.path, 'output_skip'))..createSync();
      final generator = await MasonexGenerator.fromBrick(Brick.path(brickDir.path));
      final target = DirectoryGeneratorTarget(outputDir);
