...
void main() {
  group('BricksJson', () {
    ...
    test('throws MalformedBricksJson when bricks.json is malformed', () {
      final directory = Directory.systemTemp.createTempSync();
      File(path.join(directory.path, '.masonex', 'bricks.json'))
        ..createSync(recursive: true)
        ..writeAsStringSync('[]');
      expect(
    ...
    group('clear', () {
      test('clears cache and deletes existing bricks.json', () {
        final directory = Directory.systemTemp.createTempSync();
        final bricksJson = BricksJson(directory: directory);
        final file = File(path.join(directory.path, '.masonex', 'bricks.json'))
          ..createSync(recursive: true);
    ...
    group('add', () {
      ...
      test('adds bricks to bricks.json via registry '
      ...
      test('adds bricks to bricks.json (git)', () async {
        ...
        final brick = Brick.git(
          GitPath('https://github.com/felangel/mason', path: 'bricks/simple'),
        );
        final result = await bricksJson.add(brick);
        expect(result.path, isNotEmpty);
        expect(result.brick.name, equals(brick.name));
      ...
      test('adds bricks to bricks.json via git '
          'with existing empty directory', () async {
        final directory = Directory.systemTemp.createTempSync();
        final bricksJson = BricksJson(directory: directory);
        final brick = Brick.git(
      ...
      test('throws BrickIncompatibleMasonexVersion '
...
''',
      ...
      test('throws BrickResolveVersionException when '
      ...
      test('throws BrickResolveVersionException when '
        ...
        expect(bricksJson.encode, equals('{}'));
        expect(
          () => bricksJson.add(Brick.version(name: 'example', version: 'any')),
          throwsA(isA<BrickResolveVersionException>()),
        );
      ...
      test('throws BrickNotFoundException when '
          'brick does not exist (registry)', () async {
        final directory = Directory.systemTemp.createTempSync();
        final bricksJson = BricksJson(directory: directory);
        final file = File(path.join(directory.path, '.masonex', 'bricks.json'))
        ...
        expect(
          () =>
              bricksJson.add(Brick.version(name: 'greeting', version: '0.0.0')),
      ...
      test('throws MasonexYamlNameMismatch when '
    ...
    group('rootDir', () {
      ...
      test('uses LOCALAPPDATA on windows environment when '
