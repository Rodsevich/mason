...
void main() {
  group('Hooks', () {
    ...
    group('supports programmatic usage', () {
      ...
      test('dart run', () async {
        final result = await Process.run('dart', [program, tempDir.path, name]);
      ...
      test('aot-snapshot', () async {
        ...
        expect(compileResult.exitCode, equals(ExitCode.success.code));
        final runResult = await Process.run('dartaotruntime', [
          entrypoint,
          tempDir.path,
          name,
        ]);
    ...
    test('throws HookExecutionException on IsolateSpawnException', () async {
      ...
      final files = [
        ...
        File(
          canonicalize(
            path.join(hookDirectoryPath, '.dart_tool', 'package_config.json'),
          ),
        ),
        File(canonicalize(path.join('.dart_tool', 'package_config.json'))),
      ];
    ...
    test('recompiles hooks when an IsolateSpawnException occurs', () async {
      ...
      final files = hooksBuildDirectory
      ...
      expect(preGenModule!.readAsBytesSync(), isNot(equals(legacyPreGenBytes)));
      expect(
