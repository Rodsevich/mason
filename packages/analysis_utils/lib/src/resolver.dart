...
String resolvePath(Uri uri) {
  switch (uri.scheme) {
    ...
    case "package":
      ...
      File packages = File("${getPackageRootPath()}.packages");
      Uri pathUri;
      try {
        ...
        pathUri = Uri.parse(
            packageNameLine.substring(packageNameLine.indexOf(":") + 1));
      } catch (e) {
        throw MissingPackageException(uri);
      }
      return paths.absolute(
          pathUri.path, paths.joinAll(uri.pathSegments.sublist(1)));
      break;
    default:
      throw UnimplementedError("I didn't code for '${uri.scheme}' schema");
  }
}
